...
@:enum abstract Weekday(Int) {
    ...
    @:op(A || B) @:commutative public function qyOr(b:Weekday):ValidityQuery {
        return new ValidityQuery({ mask : this | cast(b, Int), exact : false });
    }

    @:op(A && B) @:commutative public function qyAnd(b:Weekday):ValidityQuery {
...
@:forward
abstract ValidityQuery({ mask : Int, exact : Bool }) {
    public function new(query) {
        this = query;
    }

    @:op(a || b) @:commutative public function or(b:ValidityQuery):ValidityQuery {
        return new ValidityQuery({ mask : this.mask | b.mask, exact : false });
    }

    @:op(a && b) @:commutative public function and(b:ValidityQuery):ValidityQuery {
